...
# Ensure the state names are lowercase to match usmap's requirements
data$state <- tolower(data$state)

# Aggregate the data by state if it's not already aggregated
state_data <- data %>%
  group_by(state) %>%
  summarize(total_pop = sum(total_pop, na.rm = TRUE))

# Check the range of the total_pop to set appropriate limits
